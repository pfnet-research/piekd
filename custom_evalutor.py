...
def eval_performance(env, agent, n_steps, n_episodes, max_episode_len=None,
                     logger=None, step_callback=None):
    ...
    if isinstance(env, chainerrl.env.VectorEnv):
        scores = batch_run_evaluation_episodes(
            ...
            logger=logger, step_callback=step_callback)
    else:
...
def record_stats(outdir, values, suffix=''):
    with open(os.path.join(outdir, 'scores{}.txt'.format(suffix)), 'a+') as f:
...
class MultipleAgentEvaluator(object):
    def __init__(self,
                 all_agents,
                 env,
                 n_steps,
        ...
        # Write a header line first
        sampled_agent = all_agents[0]
        with open(os.path.join(self.outdir, 'scores{}.txt'.format(suffix)), 'w') as f:
            custom_columns = tuple(t[0] for t in sampled_agent.get_statistics())
            column_names = _basic_columns + ('agent_index',) + custom_columns
            print('\t'.join(column_names), file=f)

        for idx, agent in enumerate(all_agents):
            with open(os.path.join(self.outdir, 'scores-{}{}.txt'.format(idx, suffix)), 'w') as f:
                custom_columns = tuple(t[0] for t in agent.get_statistics())
                column_names = _basic_columns + ('agent_index',) + custom_columns
                print('\t'.join(column_names), file=f)
    ...
    def evaluate_and_update_max_score(self, t, episodes):
        ...
        for idx, agent in enumerate(self.all_agents):
            self.logger.info('Evaluating agent {}'.format(idx))
            eval_stats = eval_performance(
                self.env, agent, self.n_steps, self.n_episodes,
                max_episode_len=self.max_episode_len,
                logger=self.logger)
            elapsed = time.time() - self.start_time
            custom_values = tuple(tup[1] for tup in agent.get_statistics())
            ...
            values = (t,
                      ...
                      eval_stats['min'],
                      idx) + custom_values
            record_stats(self.outdir, values, '-{}{}'.format(idx, self.suffix))
            all_values.append(values)

        max_values = sorted(all_values, key=lambda x: x[3], reverse=True)[0]
        max_mean = max_values[3]
        record_stats(self.outdir, max_values, '{}'.format(self.suffix))

        if max_mean > self.max_score:
            self.logger.info('The best score is updated %s -> %s',
                             self.max_score, max_mean)
            self.max_score = max_mean
            if self.save_best_so_far_agent:
                save_agent(agent, "best", self.outdir, self.logger)
        return max_mean, [values[3] for values in all_values]

    def evaluate_if_necessary(self, t, episodes):
        if t >= self.prev_eval_t + self.eval_interval:
            score, all_scores = self.evaluate_and_update_max_score(t, episodes)
            self.prev_eval_t = t - t % self.eval_interval
